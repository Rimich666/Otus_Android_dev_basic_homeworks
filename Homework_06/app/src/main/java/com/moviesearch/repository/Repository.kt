...
import com.moviesearch.Keys.limit
import com.moviesearch.Keys.max
import com.moviesearch.Keys.pages
...
class RequestedPages(val list: List<Int>)
...
object Repository {
    ...
    fun initData(emitter: Emitter<Any>){
        ...
        if (pages == null){
            ...
            emitter.onNext(RequestedPages(pages!!))
        }
        val observable = Observable.create<Any> {emitter->
            LoadData.loadPages(pages!!, emitter)
        }
...
//                TODO("Not yet implemented")
            ...
            override fun onNext(t: Any) {
                Log.d(RXTAG, "${trace()} $t")
                when(t){
                    is RequestedPages -> emitter.onNext(t)
                    is ResultRequest -> emitter.onNext(t)
                    is Progress -> {
                        limit = t.limit
                        max = t.max
                        if (!replay) emitter.onNext(t)
                    }
                }
            }
